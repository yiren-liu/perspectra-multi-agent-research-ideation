import { ChatMessage, GraphData, LRTableData, Paper, Persona, PersonaTraitEdit, ConceptSource, TableOfContents, ForumThread, AgentInfo, PersonaProfileTemplate, Citation, BaselineResearchProposal } from "@/types";
...
interface AppState {
  ...
  setChatCitations: (citations: Citation[]) => void;
  addChatCitation: (citation: Citation) => void;
  addChatCitations: (citations: Citation[]) => void;
  removeChatCitation: (citationId: string) => void;
  clearChatCitations: () => void;

  // Research proposal for baseline group chat
  baselineResearchProposal: BaselineResearchProposal | null;
  setBaselineResearchProposal: (proposal: BaselineResearchProposal) => void;
...
export const useChatStore = create<AppState>()(
  devtools(
    persist(
      ...
      // Chat citations
      chatCitations: [],
      setChatCitations: (citations: Citation[]) => set({ chatCitations: citations }),
      addChatCitation: (citation: Citation) => set((state) => ({ 
        chatCitations: [...state.chatCitations, citation] 
      })),
      addChatCitations: (citations: Citation[]) => set((state) => ({ 
        chatCitations: [...state.chatCitations, ...citations] 
      })),
      removeChatCitation: (citationId: string) => set((state) => ({ 
        chatCitations: state.chatCitations.filter(c => c.paper_id !== citationId) 
      })),
      clearChatCitations: () => set({ chatCitations: [] }),

      // Research proposal for baseline group chat
      baselineResearchProposal: null,
      setBaselineResearchProposal: (proposal: BaselineResearchProposal) => set({ baselineResearchProposal: proposal }),

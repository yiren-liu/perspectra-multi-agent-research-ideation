...
class SelectorGroupChatManager(BaseGroupChatManager):
    ...
    async def select_speaker(self, thread: List[BaseAgentEvent | BaseChatMessage]) -> str:
        """Selects the next speaker in a group chat by asking each agent for their intent to speak
        and selecting the agent with the highest intent score.

        A key assumption is that the agent type is the same as the topic type, which we use as the agent name.
        """

        # Use the selector function if provided - maintain backward compatibility
        if self._selector_func is not None:
            ...
            if speaker is not None:
                ...
                # Skip the intent-based selection
                return speaker
        ...
        # Construct the history of the conversation for context
        ...
        # Collect intent scores from all eligible participants
        intent_scores = await self._collect_intent_scores(participants, self._participant_descriptions, history)
        
        # Find the agent with the highest intent score
        if not intent_scores:
            # Fallback if no scores were collected
            trace_logger.warning("No intent scores collected, using fallback selection.")
            if self._previous_speaker is not None and self._previous_speaker in participants:
                return self._previous_speaker
            return participants[0]
        
        # Find the agent with highest intent score
        max_score = max(intent_scores.values())
        highest_intent_agents = [agent for agent, score in intent_scores.items() if score == max_score]
        
        # If there's a tie, select randomly (or could use other tie-breaking mechanisms)
        selected_agent = highest_intent_agents[0]  # Simple selection of first highest score
        
        self._previous_speaker = selected_agent
        trace_logger.debug(f"Selected speaker based on intent scores: {selected_agent} (score: {max_score})")
        return selected_agent

    async def _collect_intent_scores(self, participants: List[str], participant_descriptions: List[str], history: str) -> Dict[str, float]:
        """Collect intent scores from all participants.
        
        Args:
            participants: List of participant names to collect scores from
            history: The conversation history
            
        Returns:
            Dict mapping participant names to their intent scores
        """
        intent_scores = {}
        intent_prompt = """Read the following conversation history and rate your intent to be the next speaker.
Provide a score from 0.0 to 10.0 where:
- 0.0 means you have nothing valuable to add to the conversation at this moment
- 10.0 means you strongly believe you should speak next because you have critical information or expertise to contribute

Only respond with a single numeric score between 0.0 and 10.0.

Conversation history:
{history}

Your intent score (0.0-10.0):"""
        
        for participant, description in zip(participants, participant_descriptions):
            score_prompt = intent_prompt.format(history=history)
            
            # Create message for model
            intent_messages: List[SystemMessage | UserMessage | AssistantMessage]
            if ModelFamily.is_openai(self._model_client.model_info["family"]):
                intent_messages = [
                    SystemMessage(content=f"You are {participant} with the following description: {description}. Evaluate your intent to speak next."),
                    UserMessage(content=score_prompt, source="user")
                ]
            else:
                intent_messages = [
                    UserMessage(content=f"You are {participant} with the following description: {description}. {score_prompt}", source="user")
                ]
            
            # Try to get a score from the model
            max_attempts = 3
            for attempt in range(max_attempts):
                try:
                    response = await self._model_client.create(messages=intent_messages)
                    ...
                    if score_match:
                        ...
                        intent_scores[participant] = score
                        trace_logger.debug(f"Participant {participant} intent score: {score}")
                        break
                    else:
                        feedback = "Please respond with a single numeric score between 0.0 and 10.0."
                        intent_messages.append(AssistantMessage(content=score_text, source=participant))
                        intent_messages.append(UserMessage(content=feedback, source="user"))
                except Exception as e:
                    trace_logger.warning(f"Error getting intent score from {participant}: {str(e)}")
                    if attempt == max_attempts - 1:
                        # On final attempt, assign a default score
                        intent_scores[participant] = 5.0  # Default middle score
        
        return intent_scores

    async def _select_speaker(self, roles: str, participants: List[str], history: str, max_attempts: int) -> str:
        """Legacy method for model-based speaker selection.
        This is kept for backwards compatibility but will be deprecated in favor of intent-based selection.
        """
        # This implementation remains as a fallback
...
class IntentBasedSelectorGroupChat(BaseGroupChat, Component[SelectorGroupChatConfig]):
    """A group chat team that have participants score their intent of being the next speaker, and then
    select the next speaker based on the highest score.
    ...
    A team with multiple participants:
        ...
        .. code-block:: python
            ...
            async def main() -> None:
                model_client = OpenAIChatCompletionClient(model=app_settings.openai_model)
